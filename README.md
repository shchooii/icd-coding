...
Our experiments follow the official data splits and preprocessing pipeline proposed by
**Edin et al. (2023)** — *“Automated Medical Coding on MIMIC-III and MIMIC-IV: A Critical Review and Replicability Study”* ([SIGIR 2023](https://doi.org/10.1145/3539618.3591918)).

### 1️⃣ Accessing MIMIC Datasets

To reproduce our results, you must first obtain access to the official MIMIC datasets:

| Dataset           | Version | Source                                                                      | Description                      |
| :---------------- | :------ | :-------------------------------------------------------------------------- | :------------------------------- |
| **MIMIC-III**     | v1.4    | [PhysioNet MIMIC-III](https://physionet.org/content/mimiciii/1.4/)          | ICD-9 codes, discharge summaries |
| **MIMIC-IV**      | v2.2    | [PhysioNet MIMIC-IV](https://physionet.org/content/mimiciv/2.2/)            | ICD-10 codes, structured data    |
| **MIMIC-IV-Note** | v2.2    | [PhysioNet MIMIC-IV-Note](https://physionet.org/content/mimic-iv-note/2.2/) | de-identified clinical notes     |

> ⚠️ Access requires completion of the [CITI training course](https://physionet.org/about/citi-course/).
> Approval is free but mandatory before downloading any MIMIC data.

---

### 2️⃣ Preparing the Data

1. **Download** the raw CSV files after access is granted.
   Place them under your preferred directory (e.g., `/data/mimiciv/` and `/data/mimiciv-note/`).

2. **Set paths** in the configurations (`configs/data/mimiciv.yaml` or `src/settings.py`):
...
3. **Run the preprocessing scripts** to generate clean splits and tokenized datasets:
   ...
   These scripts automatically produce train/validation/test splits following the Edin et al. protocol (≈ 8:1:1 ratio).

---

### 3️⃣ Reproducing Baselines

If you wish to compare against classical models (CAML, LAAT, MultiResCNN, etc.),
refer to the official SIGIR 2023 repository:
👉 [link](https://github.com/JoakimEdin/medical-coding-reproducibility)
